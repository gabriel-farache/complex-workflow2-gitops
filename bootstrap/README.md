...
Before applying the manifests, there is a need to replace the `__REPLACE_SSH_PRIVATE_KEY__` string in complex.workflow2-argocd-repo.yaml with the SSH key to enable access to the Git repository.

Either manually edit the file or login to the target cluster and run the following command:

```
git clone https://github.com/gabriel-farache/complex-workflow2-gitops.git
cd complex-workflow2-gitops/bootstrap
...
sed -i "s/__REPLACE_SSH_PRIVATE_KEY__/$SSH_PRIVATE_KEY/" complex.workflow2-argocd-repo.yaml
...
```
git clone https://<PAT>@github.com/gabriel-farache/complex-workflow2-gitops.git
```
